...
class Quanvolution:
    ...
    def get_bound_circuit(self, qc, theta, theta_vals):
        bound_qc = qc.bind_parameters({param: val for param, val in zip(theta, theta_vals)})
        return bound_qc

    def run_bound_circuits(self, bound_qcs: QuantumCircuit | list[QuantumCircuit], shots=1024):
        counts = execute(bound_qcs, self.backend, shots=shots).result().get_counts()
        return counts
    ...
    def forward(self, t: torch.Tensor):
        ...
        for filter in self.filters:
            bound_circuits.extend([self.get_bound_circuit(*filter, t.reshape(ks2).tolist()) for t in t_blocks])

        print(f'Running {bs * iout * jout * self.nfilters} circuits...')
        t0 = time.perf_counter()
        counts = self.run_bound_circuits(bound_circuits)
        print(f'Run completed in {time.perf_counter() - t0:.2f} second.')
        n_blocks = len(counts) // self.nfilters
        counts = [counts[i : i + n_blocks] for i in range(0, len(counts), n_blocks)]
        expectations = [
            [self.compute_expectation(count, self.edges[i]) for count in counts[i]] for i in range(self.nfilters)
        ]
        result = [torch.tensor(filter_expectations).reshape(bs, iout, jout) for filter_expectations in expectations]
